...
def FindModuleSpace():
  ...
  while True:
    module_space = os.path.dirname(stub_filename) + '/runfiles'
    print(os.path.dirname(stub_filename) + '.runfile')
...
def ExtractZip(zip_path, dest_dir):
  ...
  if True:
    if True:
      os.system("unzip -qq " + zip_path + " -d " + dest_dir)
      return
...
def GetRepositoriesImports(module_space, import_all):
  ...
  return [os.path.join(module_space, 'com_google_deepvariant')]
...
""".format(
    target='//deepvariant:make_examples',
    ret_code=ret_code,
...
def Main():
  ...
  rel_path = 'com_google_deepvariant/deepvariant/make_examples.py'
  if IsWindows():
  ...
  try:
    ...
    if IsRunningFromZip():
      ...
      if os.environ.get('RUN_UNDER_RUNFILES') == '1':
        os.chdir(os.path.join(module_space, 'com_google_deepvariant'))
